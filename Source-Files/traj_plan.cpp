...
double*** traj_plan::discreteTrajectory(double** viaPoints, int numVia, int numPoints) 
{
	...
	for (int i = 0; i < 4; i++)
	{ //each joint value is in the columns of viaPoints
		for (int j = 0; j < 5; j++) 
		{
		...
		if (i == 0 || i == 1 || i == 3) 
		{
			...
			while ((ii <= (numPoints)) && ((int)Output[i][ii][0] != 999)) 
			{
				...
				if (Output[i][ii][1] > 180) 
				{		//changing values to the range [-180 180]
		...
		ii = 0;
		while ((ii < (numPoints)) && (Output[i][ii][0] != 999)) 
		{
...
{		//KARA change Time Resolution to a consatant value
