...
* Problem 1
*
* Class Descriptio// ConsoleApplication1.cpp : Defines the entry point for the console application.
//
...
int main() {
	...
	cout << "***** We are assuming that all input joint values are some value between -180 and 180***" << endl;
...
/*	 int i = 0;
	while (i < 8) {
		theta1 = findTheta1(Tmatrix, oldJoints);
...
//	printInternalMatrix(internal_joints);
	...
	i = 0;
	while(i < 8) {
		theta4=findTheta4(Tmatrix,oldJoints, internal_joints[i][1], internal_joints[i][2]);
...
double *findTheta1(double **Tmatrix, double* oldJoints) {
	...
	a = Tmatrix[0][3];
	b = Tmatrix[1][3];
	c = (pow(Tmatrix[0][3],2) + pow(Tmatrix[1][3], 2) + 17861) / 390;
...
n: Bidirectional convertor btw. user friendly and machine friendly code for a 2x2 rotation matrix and 2x1 position matrix
* Author: CM
* Date: Oct. 24th. 2016
*/
