...
traj_plan::traj_plan() {
	...
	joint = vector<double>(5);
	time = vector<double>(5);
	tau = vector<double>(5); //Is thie used anywhere?

	coeff = vector<double>(4);
	coefficient = vector<double>(4); //ai bi ci di
}
...
*/
vector<double> traj_plan::discreteTrajectory(vector<double> viaPoints, int numVia, int numPoints, int jointNUM)
{
	//potential for memory leak
	Output = vector<double>(4*numPoints);
